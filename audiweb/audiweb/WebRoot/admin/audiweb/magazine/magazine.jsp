...
(function(){
	Ext.define('mainModel',{
		...
		fields : ['cmId','cmName','cmGender','cmMobile','cmBirthday','cmAddtime','cmProvince','cmCity','cmAddress','cmZip','cmIscar','cmBrand','cmModel','cmFootnum']
	});
	/**store*/
	var mainModel_store = Ext.create('Ext.data.Store',{
		...
		proxy:{
			type:'ajax',
			url:'magazine_getList.action',//请求后台西信息
			method:'POST',
			reader:{
				type:'json',
				root:'list', // 根节点
	...
	function searchForm(){
		return Ext.create('Ext.form.Panel',{
			...
			items:[
				{xtype:'textfield',labelWidth:50,width:190,fieldLabel:"用户名",name:'cmName'},
	...
	function mainGrid(){
		return Ext.create('Ext.grid.Panel',{
			...
			tbar:[
				{text:"导出申请杂志用户数据",iconCls: 'icon-down',handler:function(){
						...
						Ext.Ajax.request( {
							type : 'ajax',
							timeout : 60000,
							url : 'magazine_driverExport.action',
							method : 'POST',
							params : { 
							...
							success : function(response, opts) {
								...
								if(retData.fileName){
									...
									downloadIframe.src ="magazine_downloadExcel.action?fileName="+retData.fileName;
			...
			columns:{
				...
				items:[
					...
					{header:'姓名',dataIndex:'cmName',flex:1},
					/**{header:'性别',dataIndex:'cmGender',flex:1,renderer:function(v){
						if(v=="" || v=="null" || v==null){
							return "";
						}else {
							return v==1?"男":"女";
					...
					{header:'出生日期',dataIndex:'cmBirthday',flex:1,renderer:function(v){
			       	  return getDateTime(v);
		         	}},*/
		         	{header:'手机',dataIndex:'cmMobile',flex:1},
					{header:'所在省',dataIndex:'cmProvince',flex:1,renderer:function(v,m,r){
						if(isEmpty(v)){
							return "";
						}
						return getDictPovince(v);
					}},
					{header:'所在市',dataIndex:'cmCity',flex:1,renderer:function(v,m,r){
						if(isEmpty(r)){
							return "";
						}
						return getDictCity(r.get('cmProvince'),v);
					}},
					{header:'地址',dataIndex:'cmAddress',flex:1},
					{header:'邮编',dataIndex:'cmZip',flex:1},
					{header:'是否有车',dataIndex:'cmIscar',flex:1,renderer:function(v){
						if(v=="" || v=="null" || v==null){
							return "";
						}else {
							return v==1?"有":"无";
					...
					{header:'已有车辆品牌',dataIndex:'cmBrand',flex:1,renderer:function(v,m,r){
						if(isEmpty(v)){
							return v;
						}
						return getDictBrand(v);
					}},
					{header:'已有车辆型号',dataIndex:'cmModel',flex:1,renderer:function(v,m,r){
						if(isEmpty(v)){
							return v;
						}
						return getDictBrandModel(r.get('cmBrand'),v);
					}},
		         	{header:'车架号',dataIndex:'cmFootnum',flex:1},
					{header:'创建日期',dataIndex:'cmAddtime',flex:1,renderer:function(v){
						return getDateTime(v);
					}}
			    ]
			}
