...
(function(){
	Ext.define('mainModel',{
		...
		fields : ['cpId','cpUserid','cpAddtime','cpName','cpGender','cpMobile','cpBirthday','cpDescription','cpWhen','cpTiming']
	});
	/**store*/
	var mainModel_store = Ext.create('Ext.data.Store',{
		...
		proxy:{
			type:'ajax',
			url:'proposal_getList.action',//请求后台信息
			method:'POST',
			reader:{
				type:'json',
				root:'list', // 根节点
	...
	function searchForm(){
		return Ext.create('Ext.form.Panel',{
			...
			items:[
				{xtype:'textfield',labelWidth:40,width:200,fieldLabel:"姓名",name:'cpName'},
				...
				{xtype: 'button',width:60,margin:'0 0 5 10',text:"<s:text name='util.search'/>",iconCls:'icon-find',
					handler:function(){
	...
	function mainGrid(){
		return Ext.create('Ext.grid.Panel',{
			...
			tbar:[
				{text:"导出建议信息",iconCls: 'icon-down',handler:function(){
						...
						Ext.Ajax.request( {
							type : 'ajax',
							timeout : 60000,
							url : 'proposal_proposalExport.action',
							method : 'POST',
							params : { 
							...
							success : function(response, opts) {
								...
								if(retData.fileName){
									...
									downloadIframe.src ="proposal_downloadExcel.action?fileName="+retData.fileName;
			...
			columns:{
				...
				items:[
					...
					{header:'用户ID',dataIndex:'cpUserid',flex:1,hidden:true},
					{header:'创建日期',dataIndex:'cpAddtime',flex:1,renderer:function(v){
						return getDateTime(v);
					}},
					{header:'姓名',dataIndex:'cpName',flex:1},
					{header:'性别',dataIndex:'cpGender',flex:1,renderer:function(v){
						if(null==v || "null"==v){
						}else {
							return v==1?"男":"女";
						}
					}},
					{header:'手机号码',dataIndex:'cpMobile',flex:1},
					{header:'出生日期',dataIndex:'cpBirthday',flex:1,renderer:function(v){
						return getDateTime(v);
					}},
		         	{header:'描述',dataIndex:'cpDescription',flex:1},
		         	{header:'日期选择',dataIndex:'cpWhen',flex:1,renderer:function(v){
						if(v=="" || v=="null" || v==null){
							return "";
						}
		         		if(v==1){
		         	...
		         	{header:'时间选择',dataIndex:'cpTiming',flex:1},
					{header:'创建日期',dataIndex:'cpAddtime',flex:1,renderer:function(v){
						return getDateTime(v);
					}}
			    ]
