...
/**根据mid获取车型*/
function getDictModelByMid(mid){
	if(isEmpty(mid) ){
		return "";
	}
	for(var i=0;i<autosmodel.length;i++){
		if(!isEmpty(autosmodel[i])){
			for(var j=0;j<autosmodel[i].length;j++){
				if(autosmodel[i][j][0]==mid){
					return autosmodel[i][j][1];
				}
			}
		}
	}
