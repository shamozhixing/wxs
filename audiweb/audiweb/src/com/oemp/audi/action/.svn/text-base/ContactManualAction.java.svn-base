...
@Scope("prototype")
@Controller("audi.action.ContactManualAction")
public class ContactManualAction extends AdminAction {
	@Autowired
	...
	public String getList(){
		...
		PageTools pt = audiServiceDispatcher.getContactManualService().findPages(page, limit, paramMap);
	...
	public String driverExport() {
		...
		List list = audiServiceDispatcher.getContactManualService().findByScreen(paramMap);
		// 第一步，创建一个workbook，对应一个Excel文件
		HSSFWorkbook wb = new HSSFWorkbook();
		// 第二步，在workbook中添加一个sheet,对应Excel文件中的sheet
		HSSFSheet sheet = wb.createSheet("申请手册用户列表");
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				ContactManual cm = (ContactManual) list.get(i);
				if(cm!=null){
					row.createCell((short) 0).setCellValue(CommonUtil.getCellString(cm.getCmName()));  //姓名
					row.createCell((short) 1).setCellValue(CommonUtil.getGender(cm.getCmGender()));    //性别
					row.createCell((short) 2).setCellValue(CommonUtil.getCellString(cm.getCmBirthday()));//出生日期
					row.createCell((short) 3).setCellValue(CommonUtil.getCellString(cm.getCmMobile())); //手机
					String province="";
					...
					row.createCell((short) 6).setCellValue(CommonUtil.getCellString(cm.getCmAddress()));//地址
					row.createCell((short) 7).setCellValue(CommonUtil.getCellString((cm.getCmZip())));//邮编
					row.createCell((short) 8).setCellValue(CommonUtil.yesOrNo(cm.getCmIscar()));//是否有车
					String brand="";
					if(!CmsUtils.isEmpty(cm.getCmBrand())){
						brand=cacheMap.get(cm.getCmBrand());
					}
					row.createCell((short) 9).setCellValue(brand);//已有车辆品牌
					String model="";
					if(!CmsUtils.isEmpty(cm.getCmModel())){
						model=cacheMap.get(cm.getCmModel());
					}
					row.createCell((short) 10).setCellValue(model);//已有车辆型号
					row.createCell((short) 11).setCellValue(CommonUtil.getCellString(cm.getCmFootnum()));//车架号
			...
			fileName = "manual"+DateUtil.getNowDate("yyyy-MM-dd") + ".xls";
