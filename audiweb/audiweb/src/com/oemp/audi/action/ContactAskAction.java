 ...
 * 功能描述 ：  -查询 -导出
 * @author dang
 * 访问url ： ask_index.action
 */
@SuppressWarnings("serial")
@Scope("prototype")
@Controller("audi.action.ContactAskAction")
public class ContactAskAction extends AdminAction {
	...
	public String getList(){
		...
		if(rows==null){
			...
			String caName = js.getString("caName");
			paramMap.put("caName", caName);
			paramMap.put("startTime", js.getString("startTime"));
			paramMap.put("endTime", js.getString("endTime"));
		}
		PageTools pt = audiServiceDispatcher.getContactAskService().findPages(page, limit, paramMap);
	...
	public String askExport() {
		...
		List list = audiServiceDispatcher.getContactAskService().findByScreen(paramMap);
		// 第一步，创建一个workbook，对应一个Excel文件
		HSSFWorkbook wb = new HSSFWorkbook();
		// 第二步，在workbook中添加一个sheet,对应Excel文件中的sheet
		HSSFSheet sheet = wb.createSheet("咨询信息列表");
		...
		try {
			for (int i = 0; i < list.size(); i++) {
				...
				ContactAsk cd = (ContactAsk) list.get(i);
				if(cd!=null){
					row.createCell((short) 0).setCellValue(cd.getCaName()!=null?cd.getCaName():"");//姓名
					row.createCell((short) 1).setCellValue(cd.getCaAddtime()!=null?cd.getCaAddtime().toString():""); //创建时间
					row.createCell((short) 2).setCellValue(CommonUtil.getGender(cd.getCaGender()));   //性别
					row.createCell((short) 3).setCellValue(cd.getCaMobile()!=null?cd.getCaMobile():"");							//手机号码
					row.createCell((short) 4).setCellValue(CommonUtil.getCellString(cd.getCaBirthday()));//出生日期
					row.createCell((short) 5).setCellValue(cd.getCaDescription()!=null?cd.getCaDescription():"");			
					//问题描述
					row.createCell((short) 6).setCellValue(cd.getCaWhen()!=null?(cd.getCaWhen().toString().equals("1")?"工作日":"周末"):"");		//何时联系
					row.createCell((short) 7).setCellValue(cd.getCaTiming()!=null?cd.getCaTiming():"");	//具体时间
				}
			}
			...
			fileName = "contactask"+DateUtil.getNowDate("yyyy-MM-dd") + ".xls";
