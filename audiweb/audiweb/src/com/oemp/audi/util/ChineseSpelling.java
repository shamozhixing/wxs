...
public class ChineseSpelling {
	 ...
	 public ChineseSpelling setResource(String resource) {
	 ...
	 public String getSelling(String chs) {
	  ...
	  for (int i = 0; i < chs.length(); i++) {
	   ...
	   if (key.getBytes().length == 2 || key.getBytes().length == 3) {
	 ...
	 public String getInitialSelling(String chs) {
		  String key, value;
		  buffer = new StringBuilder();
		  for (int i = 0; i < chs.length(); i++) {
		   key = chs.substring(i, i + 1);
		   if (key.getBytes().length == 2 || key.getBytes().length == 3) {
		    value = (String) convert(key).substring(0, 1).toUpperCase();
		    if (value == null) {
		     value = "unknown";
		    }
		   } else {
	 ...
	 public String getInitialSpelling() {
		  return this.getInitialSelling(this.getResource());
		 }
	 public static void main(String[] args) {
	  ChineseSpelling finder = ChineseSpelling.getInstance();
	  finder.setResource("中文字符一二三四五六123abc");
